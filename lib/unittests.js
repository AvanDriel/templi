...
module.exports.createUnitTests = async function() {
  ...
  if(shouldMakeTemplate['template-install']) {
    ...
    templateFolderLocation = `${templateCreation['folder-location']}/${templateCreation['folder-name']}`
  ...
  if(createConfigScript['package-permission']) {
    ...
    const editPackageResponse = await editPackage.editPackage(testName['test-name'], `jest --testPathIgnorePatterns=${templateFolderLocation}`, 'unit')
    const addTestPromptToPackage = await editPackage.editPackage(`${testName['test-name']}:examples`, `jest --testPathPattern=${templateFolderLocation}`, 'unit')
  }

  const installPackages = await updateModules.updateModules('jest');

  if(testName['test-name']) {
    return `You can now use 'npm run ${testName['test-name']}' to run your unit tests, and 'npm run ${testName['test-name']}:examples' to run the example tests!`
  } else {
